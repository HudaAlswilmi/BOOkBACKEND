...
const deleteAudio = async (req, res) => {
  ...
  try {
    const delet = await AudioBookModel.findOneAndDelete({ _id: id });
...
const AddCommint = async (req, res) => {
  try {
    ...
    const response = await AudioBookModel.findOneAndUpdate(
      { _id: id },
...
const getAudioLike = async (req, res) => {
  ...
  try {
    const likeBook = await UsrModel.findOne({ _id: userId }).populate(
      "likeAudio"
    );
